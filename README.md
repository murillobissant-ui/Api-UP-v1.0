# UpSysteM API v1.0.14 — PostgreSQL/Supabase
...
A partir da v1.0.14, a API mantém as logs sob duas regras simultâneas:
...
## Logs do Sistema
A partir da v1.0.14, os erros técnicos são separados dos ciclos operacionais.
...
Use `REQUIRED_EXTENSION_BUILD=upsystem-v1-cleanbase-003` no Render para bloquear versões antigas mesmo quando a numeração da versão for reduzida. A extensão DEV v1.0.14 envia o header `X-UpSystem-Build` em todas as chamadas online.
...
## Discord - preparação v1.0.14 BUILD003
...
## Pagamentos - preparação Fase 1 e Fase 2 v1.0.14 BUILD003
